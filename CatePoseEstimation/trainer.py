...
from datasets.datasets import SwinDRNetDataset, augs_test, augs_train, input_only
...
from utils.metrics_depth_restoration import (
    get_metrics_depth_restoration_inference,
    get_metrics_depth_restoration_train,
)
from utils.metrics_nocs import (
    align,
    compute_degree_cm_mAP,
    draw_detections,
    prepare_data_posefitting,
)
from utils.metrics_sem_seg import get_metrics_sem_seg


def depth_to_xyz(depthImage, fx, fy, scale_h=1.0, scale_w=1.0):
...
class SwinDRNetTrainer:
    def __init__(self, args, model, device_list, continue_ckpt_path):
        ...
        self.material_mask = {
            "transparent": args.mask_transparent,
        ...
        self.metrics_train_epoch_depth_restoration = {
            "total": [0.0 for i in range(6)],
            "instance": [0.0 for i in range(6)],
            "background": [0.0 for i in range(6)],
        }

        self.metrics_val_epoch_depth_restoration = {
            "total": [0.0 for i in range(6)],
            "instance": [0.0 for i in range(6)],
            "background": [0.0 for i in range(6)],
        }
        ...
        logging.basicConfig(
            filename=self.output_path + "/log.txt",
            level=logging.INFO,
    ...
    def prepare_data(self):
        ...
        db_train, _ = torch.utils.data.random_split(
            db_synthetic, (train_size_synthetic, len(db_synthetic) - train_size_synthetic)
        )

        train_loader = DataLoader(
            db_train, batch_size=self.batch_size, shuffle=True, num_workers=8, drop_last=True, pin_memory=True
        )
        ...
        db_val, _ = torch.utils.data.random_split(db_val, (val_size, len(db_val) - val_size))
        val_loader = DataLoader(
            db_val, batch_size=self.batch_size, shuffle=True, num_workers=8, drop_last=True, pin_memory=True
        )
    ...
    def get_loss_depth_restoration(self, syn_ds, ins_masks, pred_ds, pred_ds_initial, mode):
        ...
        normal_pred_ori, _, _ = get_surface_normal(
            pred_ds_initial, f, pred_ds.shape[2] / ori_h, pred_ds.shape[3] / ori_w
        )

        loss_d_initial = (
            l1(pred_ds_initial, syn_ds)
    ...
    def get_loss_sem_seg(self, sem_masks, pred_sem_seg, mode):
        loss = F.cross_entropy(
            pred_sem_seg, sem_masks.squeeze(1).long(), weight=None, reduction="none", ignore_index=255
        )
    ...
    def mrcnn_coords_symmetry_loss_graph_3(self, coords, pred_coords, masks, sem_masks, loss_fn, mode):
        ...
        y_true_stack = torch.cat(
            (y_true, rotated_y_true_1, rotated_y_true_2, rotated_y_true_3, rotated_y_true_4, rotated_y_true_5), dim=4
        )  ## shape: [batch, height, width, 3, 6]
        ...
        if mode == "train":
            ...
            (
                self.loss_train_iter_coord_x_loss,
                self.loss_train_iter_coord_y_loss,
                self.loss_train_iter_coord_z_loss,
            ) = final_loss
        else:
            (
                self.loss_val_iter_coord_x_loss,
                self.loss_val_iter_coord_y_loss,
                self.loss_val_iter_coord_z_loss,
            ) = final_loss
    ...
    def forward(self, sample_batched, mode):
        ...
        self.mrcnn_coords_symmetry_loss_graph_3(
            coords, pred_coords, ins_without_other_masks, sem_masks, "Soft_L1", mode
        )
        ...
        if mode == "train":
            self.loss_train_iter = (
                loss_weight_coord
                * (
                    self.loss_train_iter_coord_x_loss
                    + self.loss_train_iter_coord_y_loss
                    + self.loss_train_iter_coord_z_loss
                    + self.loss_train_iter_coord_l2_diff
                )
            ...
            self.loss_val_iter = (
                ...
                * (
                    ...
                    + self.loss_val_iter_coord_y_loss
                    + self.loss_val_iter_coord_z_loss
                    + self.loss_val_iter_coord_l2_diff
                    + self.loss_val_iter_coord_l1_diff
                )
    ...
    def save_iter_depth_restoration_metrics(self, labels, outputs, masks, mode):
        a1, a2, a3, rmse, abs_rel, mae = get_metrics_depth_restoration_train(
            labels, outputs, self.get_metrics_w, self.get_metrics_h
        )
        num_samples_train = len(self.train_loader)
        num_samples_val = len(self.val_loader)
        if mode == "train":
            ############################### depth restoration loss #########################################
            self.loss_train_epoch_depth_restoration["total"] += (
                self.loss_train_iter_depth_restoration["total"].item() / num_samples_train
            )
            self.writer.add_scalar(
                "Train/loss/Train_Loss", self.loss_train_iter_depth_restoration["total"].item(), self.total_iter_num
            )
            ...
            if self.depth_restoration_train_with_masks:
                ############################### depth restoration loss #########################################
                self.loss_train_epoch_depth_restoration["instance"] += (
                    self.loss_train_iter_depth_restoration["instance"].item() / num_samples_train
                )
                self.loss_train_epoch_depth_restoration["background"] += (
                    self.loss_train_iter_depth_restoration["background"].item() / num_samples_train
                )
                self.writer.add_scalar(
                    "Train/loss/Train_Instance_Loss",
                    self.loss_train_iter_depth_restoration["instance"].item(),
                    self.total_iter_num,
                )
                self.writer.add_scalar(
                    "Train/loss/Train_Background_Loss",
                    self.loss_train_iter_depth_restoration["background"].item(),
                    self.total_iter_num,
                )
                ...
                (
                    ...
                    instance_abs_rel,
                    instance_mae,
                ) = get_metrics_depth_restoration_train(labels, outputs, self.get_metrics_w, self.get_metrics_h, masks)
                ...
                self.metrics_train_epoch_depth_restoration["instance"][4] += (
                    instance_abs_rel.item() / num_samples_train
                )
                ...
                (
                    ...
                    labels, outputs, self.get_metrics_w, self.get_metrics_h, 1 - masks
                )
                ...
                self.metrics_train_epoch_depth_restoration["background"][3] += (
                    background_rmse.item() / num_samples_train
                )
                self.metrics_train_epoch_depth_restoration["background"][4] += (
                    background_abs_rel.item() / num_samples_train
                )
                self.metrics_train_epoch_depth_restoration["background"][5] += (
                    background_mae.item() / num_samples_train
                )
                ...
                self.writer.add_scalar(
                    "Train/metrics/Instance/Train_abs_rel", instance_abs_rel.item(), self.total_iter_num
                )
                ...
                self.writer.add_scalar(
                    "Train/metrics/Background/Train_rmse", background_rmse.item(), self.total_iter_num
                )
                self.writer.add_scalar(
                    "Train/metrics/Background/Train_abs_rel", background_abs_rel.item(), self.total_iter_num
                )
                self.writer.add_scalar(
                    "Train/metrics/Background/Train_mae", background_mae.item(), self.total_iter_num
                )

        else:
            ############################### depth restoration loss #########################################
            self.loss_val_epoch_depth_restoration["total"] += (
                self.loss_val_iter_depth_restoration["total"].item() / num_samples_val
            )
            ...
            if self.depth_restoration_train_with_masks:
                ############################### depth restoration loss #########################################
                self.loss_val_epoch_depth_restoration["instance"] += (
                    self.loss_val_iter_depth_restoration["instance"].item() / num_samples_val
                )
                self.loss_val_epoch_depth_restoration["background"] += (
                    self.loss_val_iter_depth_restoration["background"].item() / num_samples_val
                )
                ...
                self.metrics_val_epoch_depth_restoration["background"][4] += (
                    background_abs_rel.item() / num_samples_val
                )
    ...
    def save_epoch_depth_restoration_metrics(self, mode="val"):
        ...
        if self.depth_restoration_train_with_masks:
            if mode == "val":
                self.writer.add_scalar(
                    "Val/loss/Val_Instance_Loss", self.loss_val_epoch_depth_restoration["instance"], self.epoch
                )
                self.writer.add_scalar(
                    "Val/loss/Val_Background_Loss", self.loss_val_epoch_depth_restoration["background"], self.epoch
                )
        ...
        if mode == "val":
            self.writer.add_scalar(
                "Val/metrics/Val_a1", self.metrics_val_epoch_depth_restoration["total"][0], self.epoch
            )
            self.writer.add_scalar(
                "Val/metrics/Val_a2", self.metrics_val_epoch_depth_restoration["total"][1], self.epoch
            )
            self.writer.add_scalar(
                "Val/metrics/Val_a3", self.metrics_val_epoch_depth_restoration["total"][2], self.epoch
            )
            self.writer.add_scalar(
                "Val/metrics/Val_rmse", self.metrics_val_epoch_depth_restoration["total"][3], self.epoch
            )
            self.writer.add_scalar(
                "Val/metrics/Val_abs_rel", self.metrics_val_epoch_depth_restoration["total"][4], self.epoch
            )
            self.writer.add_scalar(
                "Val/metrics/Val_mae", self.metrics_val_epoch_depth_restoration["total"][5], self.epoch
            )
        ...
        if self.depth_restoration_train_with_masks:
            if mode == "val":
                self.writer.add_scalar(
                    "Val/metrics/Instance/Val_a1", self.metrics_val_epoch_depth_restoration["instance"][0], self.epoch
                )
                self.writer.add_scalar(
                    "Val/metrics/Instance/Val_a2", self.metrics_val_epoch_depth_restoration["instance"][1], self.epoch
                )
                self.writer.add_scalar(
                    "Val/metrics/Instance/Val_a3", self.metrics_val_epoch_depth_restoration["instance"][2], self.epoch
                )
                self.writer.add_scalar(
                    "Val/metrics/Instance/Val_rmse",
                    self.metrics_val_epoch_depth_restoration["instance"][3],
                    self.epoch,
                )
                self.writer.add_scalar(
                    "Val/metrics/Instance/Val_abs_rel",
                    self.metrics_val_epoch_depth_restoration["instance"][4],
                    self.epoch,
                )
                self.writer.add_scalar(
                    "Val/metrics/Instance/Val_mae", self.metrics_val_epoch_depth_restoration["instance"][5], self.epoch
                )
            ...
            if mode == "val":
                self.writer.add_scalar(
                    "Val/metrics/Background/Val_a1",
                    self.metrics_val_epoch_depth_restoration["background"][0],
                    self.epoch,
                )
                self.writer.add_scalar(
                    "Val/metrics/Background/Val_a2",
                    self.metrics_val_epoch_depth_restoration["background"][1],
                    self.epoch,
                )
                self.writer.add_scalar(
                    "Val/metrics/Background/Val_a3",
                    self.metrics_val_epoch_depth_restoration["background"][2],
                    self.epoch,
                )
                self.writer.add_scalar(
                    "Val/metrics/Background/Val_rmse",
                    self.metrics_val_epoch_depth_restoration["background"][3],
                    self.epoch,
                )
                self.writer.add_scalar(
                    "Val/metrics/Background/Val_abs_rel",
                    self.metrics_val_epoch_depth_restoration["background"][4],
                    self.epoch,
                )
                self.writer.add_scalar(
                    "Val/metrics/Background/Val_mae",
                    self.metrics_val_epoch_depth_restoration["background"][5],
                    self.epoch,
                )
    ...
    def save_iter_sem_seg_metrics(self, labels, outputs, mode):
        ...
        if mode == "train":
            ...
            self.writer.add_scalar(
                "Train/loss/Train_Loss_Sem_Seg", self.loss_train_iter_sem_seg.item(), self.total_iter_num
            )
    ...
    def save_iter_coord_metrics(self, mode):
        ...
        if mode == "train":
            ...
            self.writer.add_scalar(
                "Train/loss/Train_Loss_Coord_x", self.loss_train_iter_coord_x_loss.item(), self.total_iter_num
            )
            self.writer.add_scalar(
                "Train/loss/Train_Loss_Coord_y", self.loss_train_iter_coord_y_loss.item(), self.total_iter_num
            )
            self.writer.add_scalar(
                "Train/loss/Train_Loss_Coord_z", self.loss_train_iter_coord_z_loss.item(), self.total_iter_num
            )
            self.writer.add_scalar(
                "Train/loss/Train_Loss_Coord_l1", self.loss_train_iter_coord_l1_diff.item(), self.total_iter_num
            )
            self.writer.add_scalar(
                "Train/loss/Train_Loss_Coord_l2", self.loss_train_iter_coord_l2_diff.item(), self.total_iter_num
            )
        else:
    ...
    def inference(self):
        ...
        for i, batch in enumerate(tqdm(self.val_loader)):
            ...
            self.inference_iter_coord_metrics(
                self.transfer_to_cpu(batch), output_sem_seg.detach().cpu(), output_coords.detach().cpu(), mode="val"
            )
    ...
    def inference_iter_coord_metrics(self, batch, output_sem_masks, output_coords, mode):
        ...
        for i in range(self.batch_size):
            ...
            pred_class_ids, pred_scores, pred_masks, pred_coords, pred_boxes = prepare_data_posefitting(
                resize_output_sem_masks, resize_output_coords
            )

            gt_class_ids, gt_scores, gt_masks, gt_coords, gt_boxes = prepare_data_posefitting(
                resize_sem_masks, resize_coords
            )
            ...
            result["gt_RTs"], scales, error_message, _ = align(
                gt_class_ids, gt_masks, gt_coords, resize_syn_ds, intrinsics, synset_names
            )
            ...
            result["pred_RTs"], result["pred_scales"], error_message, elapses = align(
                pred_class_ids, pred_masks, pred_coords, resize_output_ds, intrinsics, synset_names, if_norm=True
            )
