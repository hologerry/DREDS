...
def compute_RT_degree_cm_symmetry(RT_1, RT_2, class_id, handle_visibility, synset_names):
    ...
    if synset_names[class_id] in ["bottle", "can", "bowl"]:  ## symmetric when rotating around y-axis
        ...
        if synset_names[class_id] == "can":
            theta1 = np.arccos(y1.dot(y2) / (np.linalg.norm(y1) * np.linalg.norm(y2)))
            y2 = -y2
            theta2 = np.arccos(y1.dot(y2) / (np.linalg.norm(y1) * np.linalg.norm(y2)))
...
def compute_RT_projection_2d_symmetry(
    RT_1, RT_2, class_id, handle_visibility, mesh_vertices, intrinsics, synset_names, num_rotation=20
):
    """
    :param RT_1: [4, 4]. homogeneous affine transformation
    :param RT_2: [4, 4]. homogeneous affine transformation
...
def get_3d_bbox_new(scale, shift=0):
    """
    Input:
        scale: [3] or scalar
        shift: [3] or scalar
...
def rotate_and_crop(image, rotate_degree, interpolation):
    ...
    image_rotated_cropped = crop_around_center(
        image_rotated, *largest_rotated_rect(image_width, image_height, math.radians(rotate_degree))
    )
...
def rotate_and_crop_images(image, masks, coords, rotate_degree):
    ...
    image_rotated_cropped = cv2.resize(
        image_rotated_cropped, (image_width, image_height), interpolation=cv2.INTER_LINEAR
    )
    mask_rotated_cropped = cv2.resize(
        mask_rotated_cropped, (image_width, image_height), interpolation=cv2.INTER_NEAREST
    )
    ...
    if coords is not None:
        ...
        coord_rotated_cropped = cv2.resize(
            coord_rotated_cropped, (image_width, image_height), interpolation=cv2.INTER_NEAREST
        )
...
def compute_ap_from_matches_scores(pred_match, pred_scores, gt_match):
    ...
    acc = np.sum(pred_match > -1) / len(pred_match)
    # return ap
    return acc
...
def compute_RT_projection_2d_overlaps(
    gt_class_ids, gt_RTs, gt_handle_visibility, pred_class_ids, pred_RTs, meshes, intrinsics, synset_names
):
    """Finds overlaps between prediction and ground truth instances.
    Returns:
        overlaps: [pred_boxes, gt_boxes] IoU overlaps.
...
def compute_iou_mAP(
    final_results, synset_names, iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False
):
    ...
    for progress, result in enumerate(final_results):
        ...
        for cls_id in range(1, num_classes):
            ...
            if use_matches_for_pose:
                ...
                cls_gt_RTs = (
                    cls_gt_RTs[iou_thres_gt_match > -1] if len(iou_thres_gt_match) > 0 else np.zeros((0, 4, 4))
                )
                cls_gt_handle_visibility = (
...
def compute_degree_cm_mAP(
    ...
    for progress, result in enumerate(final_results):
        ...
        for i in gt_class_ids :
            if i in pred_class_ids :
                index1 = pred_class_ids.tolist().index(i)
                index2 = gt_class_ids.tolist().index(i)

                pred_RTs[index1,:] = gt_RTs[index2,:]
        """
        # print(pred_bboxes.shape[0], pred_class_ids.shape[0], pred_scores.shape[0], pred_RTs.shape[0])
    ...
    pose_dict_pkl_path = os.path.join(
        ...
        + "AP_{}-{}degree_{}-{}cm.pkl".format(
            degree_thres_list[0], degree_thres_list[-2], shift_thres_list[0], shift_thres_list[-2]
        ),
    )
    pose_dict = {}
    ...
    for cls_id in range(1, num_classes):
        ...
        output_path = os.path.join(
            ...
            + "AP_{}_{}-{}degree_{}-{}cm.png".format(
                class_name, degree_thres_list[0], degree_thres_list[-2], shift_thres_list[0], shift_thres_list[-2]
            ),
        )
        plt.colorbar()
    ...
    output_path = os.path.join(
        log_dir, prefix + "mAP_{}-{}degree.png".format(degree_thres_list[0], degree_thres_list[-2])
    )
    ...
    iou_aps = iou_3d_aps
    for i in range(len(pose_aps) - 1):
        print(synset_names[i])
        print("3D IoU at 25: {:.1f}".format(iou_aps[i, iou_thres_list.index(0.25)] * 100))
        print("3D IoU at 50: {:.1f}".format(iou_aps[i, iou_thres_list.index(0.5)] * 100))
        print("5 degree, 2cm: {:.1f}".format(pose_aps[i, degree_thres_list.index(5), shift_thres_list.index(2)] * 100))
        print("5 degree, 5cm: {:.1f}".format(pose_aps[i, degree_thres_list.index(5), shift_thres_list.index(5)] * 100))
        print(
            "5 degree, 10cm: {:.1f}".format(pose_aps[i, degree_thres_list.index(5), shift_thres_list.index(10)] * 100)
        )
        print(
            "10 degree, 2cm: {:.1f}".format(pose_aps[i, degree_thres_list.index(10), shift_thres_list.index(2)] * 100)
        )
        print(
            "10 degree, 5cm: {:.1f}".format(pose_aps[i, degree_thres_list.index(10), shift_thres_list.index(5)] * 100)
        )
        print(
            "10 degree, 10cm: {:.1f}".format(
                pose_aps[i, degree_thres_list.index(10), shift_thres_list.index(10)] * 100
            )
        )
        print(
            "15 degree, 5cm: {:.1f}".format(pose_aps[i, degree_thres_list.index(15), shift_thres_list.index(5)] * 100)
        )
        print(
            "15 degree, 10cm: {:.1f}".format(
                pose_aps[i, degree_thres_list.index(15), shift_thres_list.index(10)] * 100
            )
        )
    print("mean")
    print("3D IoU at 25: {:.1f}".format(iou_aps[-1, iou_thres_list.index(0.25)] * 100))
    ...
    print("5 degree, 10cm: {:.1f}".format(pose_aps[-1, degree_thres_list.index(5), shift_thres_list.index(10)] * 100))
    ...
    print(
        "10 degree, 10cm: {:.1f}".format(pose_aps[-1, degree_thres_list.index(10), shift_thres_list.index(10)] * 100)
    )
    print("15 degree, 5cm: {:.1f}".format(pose_aps[-1, degree_thres_list.index(15), shift_thres_list.index(5)] * 100))
    print(
        "15 degree, 10cm: {:.1f}".format(pose_aps[-1, degree_thres_list.index(15), shift_thres_list.index(10)] * 100)
    )
...
def compute_2D_projection_mAP(
    ...
    for progress, result in enumerate(final_results):
        ...
        for cls_id in range(1, num_classes):
            ...
            projection_cls_gt_match, projection_cls_pred_match = compute_match_from_projection_2d_dist(
                RT_projection_2d_overlaps, cls_pred_class_ids, cls_gt_class_ids, projection_thres_list
            )
            ...
            assert (
                projection_pred_scores_all[cls_id].shape[1] == projection_pred_matches_all[cls_id].shape[1]
            ), "{} vs. {}".format(projection_pred_scores_all[cls_id].shape, projection_pred_matches_all[cls_id].shape)
    ...
    projection_dict_pkl_path = os.path.join(
        log_dir, prefix + "Projection_2D_AP_{}-{}pixel.pkl".format(projection_thres_list[0], projection_thres_list[-1])
    )
    pose_dict = {}
    ...
    output_path = os.path.join(
        log_dir, prefix + "Projection_2D_AP_{}-{}pixel.png".format(projection_thres_list[0], projection_thres_list[-1])
    )
    fig_pose.legend()
...
def compute_ap_range(
    ...
    for iou_threshold in iou_thresholds:
        ap, precisions, recalls, overlaps = compute_ap(
            gt_box, gt_class_id, gt_mask, pred_box, pred_class_id, pred_score, pred_mask, iou_threshold=iou_threshold
        )
        if verbose:
...
def prepare_data_posefitting(sem_mask, coord):
    # sem_mask : h * w
    # coord 3 * h * w

    h, w = sem_mask.shape

    class_ids = np.unique(sem_mask)

    assert class_ids[-1] == 8
    class_ids = np.delete(class_ids, -1)
    if len(class_ids) == 0:
        return (
            np.zeros((0)),
    ...
    num_instance = len(class_ids)

    masks = np.zeros([h, w, num_instance], dtype=np.uint8)
    coords = np.zeros((h, w, num_instance, 3), dtype=np.float32)
    scores = np.ones((num_instance,), dtype=np.float32)
    for i in range(num_instance):
        inst_mask = np.equal(sem_mask, class_ids[i])
        masks[:, :, i] = inst_mask

        coords[:, :, i, :] = np.multiply(coord, np.expand_dims(inst_mask, axis=-1))

    boxes = extract_bboxes(masks)
    coords = np.clip(coords, 0, 1)

    return class_ids, scores, masks, coords, boxes
...
def draw(img, imgpts, axes, color):
    ...
    for i, j in zip([4, 5, 6, 7], [5, 7, 4, 6]):
        img = cv2.line(img, tuple(imgpts[i]), tuple(imgpts[j]), color_ground, 1)

    # draw pillars in blue color
    color_pillar = (int(color[0] * 0.6), int(color[1] * 0.6), int(color[2] * 0.6))
    for i, j in zip(range(4), range(4, 8)):
        img = cv2.line(img, tuple(imgpts[i]), tuple(imgpts[j]), color_pillar, 1)
    ...
    # draw axes
    img = cv2.line(img, tuple(axes[0]), tuple(axes[1]), (0, 0, 255), 1)
    img = cv2.line(img, tuple(axes[0]), tuple(axes[3]), (255, 0, 0), 1)
    img = cv2.line(img, tuple(axes[0]), tuple(axes[2]), (0, 255, 0), 1)  ## y last
...
def draw_text(draw_image, bbox, text, draw_box=False):
    ...
    if draw_box:
        cv2.rectangle(draw_image.copy(), (bbox[1], bbox[0]), (bbox[3], bbox[2]), (255, 0, 0), 2)
    ...
    cv2.putText(draw_image, text, text_pos, fontFace, fontScale, (255, 255, 255), thickness)

    return draw_image


def cv2_resize_write(output_path, image):
...
def draw_detections(
    ...
    if draw_gt:
        ...
        cv2_resize_write(output_path, draw_image[:, :, ::-1])
        ...
        if gt_RTs is not None:
            for ind, RT in enumerate(gt_RTs):
                ...
                # print('gt_RT',RT)
                # print('gt_transformed_bbox_3d',transformed_bbox_3d)
                projected_bbox = calculate_2d_projections(transformed_bbox_3d, intrinsics)
                draw_image_bbox = draw(draw_image_bbox, projected_bbox, projected_axes, (255, 0, 0))

        cv2_resize_write(output_path, draw_image_bbox[:, :, ::-1])
    ...
    if draw_pred:
        ...
        if draw_rgb_coord:
            cv2_resize_write(output_path_r, r_image[:, :, ::-1])
            cv2_resize_write(output_path_g, g_image[:, :, ::-1])
            cv2_resize_write(output_path_b, b_image[:, :, ::-1])
        ...
        if gt_class_ids is not None:
            gt_match, pred_match, _, pred_indices = compute_matches(
                gt_bbox, gt_class_ids, gt_mask, pred_bbox, pred_class_ids, pred_scores, pred_mask, 0.5
            )
        ...
        for ind in range(num_pred_instances):
            ...
            transformed_bbox_3d = transform_coordinates_3d(bbox_3d, RT)
            # print('pred_RT',RT)
            # print('pred_transformed_bbox_3d',transformed_bbox_3d)
            projected_bbox = calculate_2d_projections(transformed_bbox_3d, intrinsics)
            draw_image_bbox = draw(draw_image_bbox, projected_bbox, projected_axes, (255, 0, 0))
...
def backproject(depth, intrinsics, instance_mask):
    ...
    pts = xyz_img[idxs[0], idxs[1], :].reshape(-1, 3)
    # shape: height * width
        ...
        if coord_pts.shape[0] <= 3:
            ...
            try:
                ...
                if save_path is not None:
                    ...
                    np.savetxt(save_path + "_{}_{}_depth_pts.txt".format(i, class_name), out)
                ...
                print("[ Error ] aligning instance", str(e))
                aligned_RT = np.identity(4, dtype=np.float32)
...
def align_ICP(
    class_ids, masks, depth, intrinsics, synset_names, image_path, save_path=None, if_norm=False, verbose=False
):
    num_instances = len(class_ids)
    error_messages = ""
    ...
    for i in range(num_instances):
        ...
        # try:
        if True:
            scales, rotation, translation = ICP.doICP(shape_pts, depth_pts, threshold=5, isViz=False)
