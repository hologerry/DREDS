...
augs_train = iaa.Sequential(
    [
        ...
        iaa.Sometimes(
            ...
            iaa.blend.Alpha(
                factor=(0.2, 0.7),
                first=iaa.blend.SimplexNoiseAlpha(
                    first=iaa.Multiply((1.5, 3.0), per_channel=False), upscale_method="cubic", iterations=(1, 2)
                ),
                name="simplex-blend",
            ),
        ),
        ...
        iaa.Sometimes(
            ...
            iaa.SomeOf(
                ...
                [
                    iaa.OneOf(
                        [iaa.MotionBlur(k=3, name="motion-blur"), iaa.GaussianBlur(sigma=(0.5, 1.0), name="gaus-blur")]
                    ),
                    iaa.OneOf(
                        [
...
augs_test = iaa.Sequential(
    [
        iaa.Resize({"height": 224, "width": 224}, interpolation="nearest"),
    ]
)
...
def load_meta(file_path):
    ...
    while len(meta) < 10:
        ...
        instance["scale"] = -1.0
        instance["material"] = -1
        instance["quaternion"] = np.array([0.0, 0.0, 0.0, 0.0])
        instance["translation"] = np.array([0.0, 0.0, 0.0])
        meta.append(instance)
    return meta
...
class SwinDRNetDataset(Dataset):
    ...
    def __init__(
        ...
        self.output_depth_dir = output_depth_dir
    ...
    def __getitem__(self, index):
        ...
        output_depth_path = self._datalist_output_depth[index]
        _output_depth = cv2.imread(output_depth_path, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)  # (360,640)
        ...
        if self.transform:
            ...
            _sim_depth = det_tf_only_resize.augment_image(
                _sim_depth, hooks=ia.HooksImages(activator=self._activator_masks)
            )
            ...
            _syn_depth = det_tf_only_resize.augment_image(
                _syn_depth, hooks=ia.HooksImages(activator=self._activator_masks)
            )
            _syn_depth = _syn_depth.transpose((2, 0, 1))  # To Shape: (1, H, W)

            _output_depth = _output_depth.transpose((1, 2, 0))  # To Shape: (H, W, 1)
            _output_depth = det_tf_only_resize.augment_image(
                _output_depth, hooks=ia.HooksImages(activator=self._activator_masks)
            )
        ...
        for i in range(len(_meta)):
            ...
            if _meta[i]["instance_folder"] != " ":
                bbox_file = os.path.join(self.obj_dir, _meta[i]["instance_folder"], _meta[i]["name"], "bbox.txt")
        ...
        _scale = torch.from_numpy(_scale)
        ...
        cache = (
            ...
            _mask_sem_tensor,
            _mask_ins_tensor,
            _mask_ins_without_other_tensor,
        )
        sample = {
            "rgb": _rgb_tensor,
    ...
    def _create_lists_filenames(
        self, rgb_dir, sim_depth_dir, output_depth_dir, syn_depth_dir, nocs_dir, mask_dir, meta_dir
    ):
        ...
        assert os.path.isdir(
            sim_depth_dir
        ), 'Dataloader given simulation depth images directory that does not exist: "%s"' % (sim_depth_dir)
        ...
        if num_sim_depth == 0:
            raise ValueError(
                "No simulation depth images found in given directory. Searched in dir: {} ".format(
        ...
        # generate output depth image paths list
        assert os.path.isdir(
            output_depth_dir
        ), 'Dataloader given output depth images directory that does not exist: "%s"' % (sim_depth_dir)
        for ext in self._extension_sim_depth:
            output_depth_search_str = os.path.join(output_depth_dir, "*/*" + ext)
            output_depth_paths = sorted(glob.glob(output_depth_search_str))
            self._datalist_output_depth = self._datalist_output_depth + output_depth_paths
        num_output_depth = len(self._datalist_output_depth)
        if num_output_depth == 0:
            raise ValueError(
                "No output depth images found in given directory. Searched in dir: {} ".format(sim_depth_search_str)
            )
        if num_output_depth != num_rgb:
            raise ValueError(
                "The number of output depth images and rgb images do not match. Please check data,"
                + "found {} output depth images and {} rgb images in dirs:\n".format(num_sim_depth, num_rgb)
                + "output depth images: {}\nrgb images: {}\n".format(syn_depth_dir, rgb_dir)
            )
        ...
        assert os.path.isdir(
            syn_depth_dir
        ), 'Dataloader given synthetic depth images directory that does not exist: "%s"' % (syn_depth_dir)
    ...
    def compute_xyz(self, depth_img, camera_params):
        ...
        indices = np.indices((int(camera_params["yres"]), int(camera_params["xres"])), dtype=np.float32).transpose(
            1, 2, 0
        )
        z_e = depth_img
