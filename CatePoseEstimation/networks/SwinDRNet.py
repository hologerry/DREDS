...
from .UPerNet import FCNHead, UPerHead
...
class SwinDRNet(nn.Module):
    ...
    def __init__(self, config, img_size=224, num_classes=3):
        ...
        self.backbone_rgb_branch = SwinTransformerSys(
        ...
        self.backbone_xyz_branch = SwinTransformerSys(
            img_size=config.DATA.IMG_SIZE,
            patch_size=config.MODEL.SWIN.PATCH_SIZE,
            in_chans=config.MODEL.SWIN.IN_CHANS,
            embed_dim=config.MODEL.SWIN.EMBED_DIM,
        ...
        self.cross_attention_0 = CrossAttention(
            in_channel=96, depth=config.MODEL.SWIN.DEPTHS[0], num_heads=config.MODEL.SWIN.NUM_HEADS[0]
        )
        self.cross_attention_1 = CrossAttention(
            in_channel=192, depth=config.MODEL.SWIN.DEPTHS[1], num_heads=config.MODEL.SWIN.NUM_HEADS[1]
        )
        self.cross_attention_2 = CrossAttention(
            in_channel=384, depth=config.MODEL.SWIN.DEPTHS[2], num_heads=config.MODEL.SWIN.NUM_HEADS[2]
        )
        self.cross_attention_3 = CrossAttention(
            in_channel=768, depth=config.MODEL.SWIN.DEPTHS[3], num_heads=config.MODEL.SWIN.NUM_HEADS[3]
        )
