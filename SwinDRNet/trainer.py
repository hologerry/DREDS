...
from utils.loss_functions import GradientLoss, weight_reduce_loss
...
class SwinDRNetTrainer:
    ...
    def forward(self, sample_batched, mode):
        ...
        pred_ds, pred_ds_initial, confidence_sim_ds, confidence_initial = self.model(
            rgbs, sim_ds
        )  # [bs, 150, 512, 512], [bs, 150, 512, 512])
    ...
    def save_iter_coord_metrics(self, mode):
        ...
        if mode == "train":
            ...
            self.writer.add_scalar(
                "Train/loss/Train_Loss_Coord", self.loss_train_iter_coord.item(), self.total_iter_num
            )
        else:
    ...
    def train(self):
        for epoch in self.iterator:
            ...
            for _, batch in enumerate(tqdm(self.train_loader)):
                ...
                outputs_depth, normal_labels, normal_pred, confidence_sim_ds, confidence_initial = self.forward(
    ...
    def inference(self):
        ...
        metrics_instance = {"a1": 0.0, "a2": 0.0, "a3": 0.0, "rmse": 0.0, "rel": 0.0, "mae": 0.0}

        metrics_background = {"a1": 0.0, "a2": 0.0, "a3": 0.0, "rmse": 0.0, "rel": 0.0, "mae": 0.0}
        ...
        for i, batch in enumerate(tqdm(self.val_loader)):
            ...
            with torch.no_grad():
                ...
                a1, a2, a3, rmse, rel, mae, num_valid = get_metrics_depth_restoration_inference(
                    sample_batched["syn_depth"],
                    outputs_depth,
                    self.get_metrics_w,
                    self.get_metrics_h,
