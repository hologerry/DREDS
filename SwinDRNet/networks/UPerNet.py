...
class PPM(nn.ModuleList):
    ...
    def __init__(self, pool_scales, in_channels, channels, align_corners):
        ...
        for pool_scale in pool_scales:
            self.append(
                nn.Sequential(nn.AdaptiveAvgPool2d(pool_scale), ConvModule(self.in_channels, self.channels, 1))
            )
...
class UPerHead(nn.Module):
    ...
    def __init__(
        ...
        # PSP Module
        self.psp_modules = PPM(pool_scales, self.in_channels[-1], self.channels, align_corners=self.align_corners)
        ...
        for in_channels in self.in_channels[:-1]:  # skip the top layer
            ...
            fpn_conv = ConvModule(self.channels, self.channels, 3, padding=1, inplace=False)
...
class FCNHead(nn.Module):
    ...
    def __init__(
        ...
        convs = []
        convs.append(ConvModule(self.in_channels, self.channels, kernel_size=kernel_size, padding=kernel_size // 2))
        for i in range(num_convs - 1):
            convs.append(ConvModule(self.channels, self.channels, kernel_size=kernel_size, padding=kernel_size // 2))
        if num_convs == 0:
